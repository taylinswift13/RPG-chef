...
public class ParabolaController : MonoBehaviour
{
 ...
 // Use this for initialization
    void Start()
    {
        ...
        GetComponent<LineRenderer>().material = Resources.Load<Material>("Material/Test");
    }
    ...
    {
        float   RT_Value;
        RT_Value = Input.GetAxis("RT");
        OnDraw();
        if (PickUpManager.instance.ObjectsInRange.Count > 0 && GameObject.Find("HoldPlace").transform.childCount==0)
        {
            Object = PickUpManager.instance.ObjectsInRange[0];
        }

        if (GetComponent<PickedUpChecking>().PickedUp)//only picked up objects can be thrown away
        {
           if (Input.GetMouseButtonUp(1) || RT_Value>0.1)
            {
                GameObject.Find("CookingSounds").GetComponent<AudioSource>().PlayOneShot(Resources.Load<AudioClip>("Sound/Throwing"));
                GameObject.Find("Body").GetComponent<Animator>().SetBool("Holding", false);
                if (!GameObject.Find("Body").GetComponent<Animator>().GetBool("Holding"))
                {
                    GameObject.Find("Body").GetComponent<Animator>().SetBool("Throwing", true);
                }
                transform.parent.parent = null;
                CookingManager.instance.InstantiatedItems.Clear();
                GetComponent<PickedUpChecking>().PickedUp = false;
                RefreshTransforms(Speed);
                FollowParabola();
                gameObject.transform.parent.GetChild(1).GetComponent<AimingController>().MinDistance = gameObject.transform.parent.GetChild(1).GetComponent<AimingController>().StartDistance;
                gameObject.transform.parent.GetChild(1).GetComponent<AimingController>().Height = gameObject.transform.parent.GetChild(1).GetComponent<AimingController>().StartHeight;
            }
        }
        ...
        if (!GetComponent<PickedUpChecking>().PickedUp)
        {
            FoodTrail.SetActive(true);
            FoodTrail.transform.position = gameObject.transform.position;
        }
        else
        {
            FoodTrail.SetActive(false);
        }
        if (PickUpManager.instance.ObjectsInRange.Count == 0) return;
    }
    ...
    public void OnDraw()
    {
        ...
        if (GetComponent<PickedUpChecking>().PickedUp) 
        {
            ...
            int accur =50;
            GetComponent<LineRenderer>().positionCount = accur;
            GetComponent<LineRenderer>().SetWidth(0.1f, 0.1f);
            
            prevPos = gizmo.Points[0].position;
            
            for (int c = 1; c <= accur; c++)
            {
                float currTime = c * gizmo.GetDuration() / accur;
                currPos = gizmo.GetPositionAtTime(currTime);
                GetComponent<LineRenderer>().SetPosition(c - 1, currPos);
                prevPos = currPos;
            }
            //render the sphere in the end of the line
           
                GameObject.Find("Sphere").transform.localPosition = currPos;
                GameObject.Find("Sphere").GetComponent<MeshRenderer>().material= Resources.Load<Material>("Material/Test");
            
        }
        else
        {
            GameObject.Find("Sphere").transform.localPosition = new Vector3(-150, -15, -20);
           GetComponent<LineRenderer>().SetVertexCount(0);
        }
        //GameObject.Find("Sphere").GetComponent<MeshRenderer>().enabled = false;
        if (PickUpManager.instance.ObjectsInRange == null) { return; }
    }
